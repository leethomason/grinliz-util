...
inline uint32_t RotLeft(uint32_t value, int r, int nBits = 32) { 
	if (nBits == 32)
		return (value << r) | (value >> (nBits - r)); 
	return ((value << r) | (value >> (nBits - r))) & ((1 << nBits) - 1);
}

inline uint32_t RotRight(uint32_t value, int r, int nBits = 32) {
	if (nBits == 32)
		return (value >> r) | (value << (nBits - r));
	return ((value >> r) | (value << (nBits - r))) & ((1 << nBits) - 1);
}
